...
async function loadDownload(detailPath, id, isSeries, se, ep) {
  ...
  try {
    ...
    const dl = d.downloads || [];
    if (dl.length) {
      links.innerHTML = dl.map((x) => `<a class="btn btn-dl" target="_blank" href="${x.url}">⬇ ${x.resolution ? x.resolution + "p" : "MP4"} ${x.format || ""} ${x.size ? '(' + Math.round(x.size/1048576) + 'MB)' : ''}</a>`).join("");
